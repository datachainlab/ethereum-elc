...
jq -n -f ${TEMPLATE_DIR}/ibc-0.json.tpl --arg MRENCLAVE ${MRENCLAVE} \
    --arg IBC_ADDRESS 0xFF00000000000000000000000000000000000000 \
    --arg EXECUTION_ENDPOINT ${EXECUTION_ENDPOINT} \
    --arg CONSENSUS_ENDPOINT ${CONSENSUS_ENDPOINT} > ${CONFIG_DIR}/ibc-0.json
# never use this config in this integration test
